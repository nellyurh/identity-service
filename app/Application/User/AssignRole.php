...
/**
 * Assign a role to a user. The role must exist (else RoleNotFound). Assignment is idempotent at
 * the aggregate; a real change bumps authz_version and emits RoleAssigned (drained to the outbox
 * with the user save). Returns the updated profile.
 */
final readonly class AssignRole
{
    ...
    public function handle(AssignRoleCommand $c): UserProfile
    {
        ...
        return $this->tx->transactional(function () use ($c, $user, $role): UserProfile {
            $user->assignRole($role->id, $this->clock->now());
